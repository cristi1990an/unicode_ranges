...
public:
		  ...
		  use_ascii_bmh_(needle.size() > 8u && details::is_ascii_only(needle))
	{
		if (use_ascii_bmh_)
		{
			...
			{
				const auto scalar = static_cast<std::uint16_t>(needle_[i]);
	...
	std::size_t find(std::u16string_view base, std::size_t pos) const noexcept
	{
		...
		const auto first_scalar = static_cast<std::uint16_t>(first);
		while (pos <= limit)
		{
...
private:
	std::size_t find_ascii_bmh(std::u16string_view base, std::size_t pos) const noexcept
	{
		const auto limit = base.size() - needle_.size();
		while (pos <= limit)
		{
			...
			const auto scalar = static_cast<std::uint16_t>(base[tail]);
...
inline constexpr bool utf16_split_delimiter_has_self_overlap(std::u16string_view delimiter) noexcept
{
...
inline constexpr bool utf16_split_delimiter_has_overlapping_predecessor(
	std::u16string_view base,
	std::u16string_view delimiter,
	...
	: public std::ranges::view_interface<basic_utf16_split_view<View, DropTrailingEmpty, DelimiterStorage>>
{
	...
	friend class basic_utf16_rsplit_view;
...
private:
		...
		std::size_t current) noexcept
	{
		const auto delimiter_pos = [&]() constexpr noexcept {
			...
			{
				return details::rfind_utf16_split_delimiter(base, delimiter, base.size());
			}
			...
			return details::rfind_utf16_split_delimiter(base, delimiter, current - 1);
		}();
		if (delimiter_pos == std::u16string_view::npos)
		{
			return 0;
		}

		if (!details::utf16_split_delimiter_has_self_overlap(delimiter)
			|| !details::utf16_split_delimiter_has_overlapping_predecessor(base, delimiter, delimiter_pos))
		{
...
template <typename View, bool DropTrailingEmpty, typename DelimiterStorage>
class basic_utf16_rsplit_view
	: public std::ranges::view_interface<basic_utf16_rsplit_view<View, DropTrailingEmpty, DelimiterStorage>>
{
public:
	static constexpr basic_utf16_rsplit_view from_delimiter_storage(
		std::u16string_view base,
		DelimiterStorage delimiter_storage) noexcept
	{
		return basic_utf16_rsplit_view{ base, delimiter_storage };
	}
	...
	public:
		...
		constexpr reference operator*() const noexcept
		{
			return View::from_code_units_unchecked(base_.substr(current_, segment_end_ - current_));
		}

		constexpr iterator& operator++() noexcept
		{
			if (current_ == 0 || current_ == std::u16string_view::npos)
			{
				current_ = std::u16string_view::npos;
				segment_end_ = std::u16string_view::npos;
				return *this;
			}

			segment_end_ = current_ - delimiter_.size();
			current_ = basic_utf16_split_view<View, DropTrailingEmpty, DelimiterStorage>::find_previous_segment_start(
				base_,
				delimiter_,
				segment_end_);
			return *this;
		...
		constexpr iterator& operator--() noexcept
		{
			...
			{
				current_ = 0;
				const auto next = details::find_utf16_split_delimiter(base_, delimiter_, 0);
				segment_end_ = next == std::u16string_view::npos ? base_.size() : next;
				return *this;
			}
			...
			const auto next = details::find_utf16_split_delimiter(base_, delimiter_, current_);
			segment_end_ = next == std::u16string_view::npos ? base_.size() : next;
			return *this;
		}
	...
	private:
		...
		std::size_t segment_end_ = std::u16string_view::npos;
	};
	...
	constexpr iterator begin() const noexcept
	{
		...
		const auto segment_end = [&]() constexpr noexcept {
			if constexpr (DropTrailingEmpty)
			{
				if (details::utf16_split_input_ends_with_delimiter(base_, delimiter))
				{
		...
		const auto current = segment_end == base_.size()
			? basic_utf16_split_view<View, DropTrailingEmpty, DelimiterStorage>::find_previous_segment_start(
				base_,
				delimiter,
				std::u16string_view::npos)
			: basic_utf16_split_view<View, DropTrailingEmpty, DelimiterStorage>::find_previous_segment_start(
...
private:
	constexpr explicit basic_utf16_rsplit_view(
		std::u16string_view base,
		DelimiterStorage delimiter_storage) noexcept
		: base_(base), delimiter_storage_(delimiter_storage)
	{}
...
template <typename View, bool DropTrailingEmpty>
using utf16_rsplit_view = basic_utf16_rsplit_view<View, DropTrailingEmpty, borrowed_utf16_split_delimiter>;

template <typename View, bool DropTrailingEmpty>
using utf16_rsplit_char_view = basic_utf16_rsplit_view<View, DropTrailingEmpty, owned_utf16_split_char_delimiter>;
	...
	const Allocator& alloc)
{
	...
	if (replacement.size() == needle.size())
	{
		...
		std::basic_string<char16_t, std::char_traits<char16_t>, Allocator> result{ source, alloc };
		std::size_t replacements = 0;
...
private:
		...
		std::size_t current) noexcept
	{
		auto previous = [&]() constexpr noexcept {
			if (current == std::u16string_view::npos)
			{
				return details::rfind_utf16_predicate_match(base, base.size(), pred);
			}

			if (current == 0)
			{
				return utf16_predicate_match{};
			}

			auto match = details::rfind_utf16_predicate_match(base, current, pred);
			while (match.pos != std::u16string_view::npos && match.pos + match.size >= current)
			{
				match = details::rfind_utf16_predicate_match(base, match.pos, pred);
			}
			return match;
		}();
		return previous.pos == std::u16string_view::npos ? 0 : previous.pos + previous.size;
	}
...
private:
	...
	static constexpr bool predicate_noexcept =
		noexcept(std::invoke(std::declval<Pred&>(), std::declval<utf16_char>()));
...
public:
	...
	[[nodiscard]]
	constexpr auto split(utf16_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		using accessor_type = details::split_view_accessor<utf16_char>;
	...
	[[nodiscard]]
	constexpr auto split_trimmed(utf16_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		using accessor_type = details::split_trimmed_view_accessor<utf16_char>;
		return details::owning_string_view<Derived, accessor_type>{
			std::move(static_cast<Derived&>(*this)),
			accessor_type{ ch }
		};
	...
	{
		return utf16_rsplit_char_view<View, false>::from_delimiter_storage(
			code_unit_view(),
			details::owned_utf16_split_char_delimiter{ ch });
	}

	[[nodiscard]]
	constexpr auto rsplit(utf16_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		using accessor_type = details::rsplit_view_accessor<utf16_char>;
		return details::owning_string_view<Derived, accessor_type>{
			std::move(static_cast<Derived&>(*this)),
			accessor_type{ ch }
		};
	...
	{
		return utf16_rsplit_view<View, false>::from_delimiter_storage(
			code_unit_view(),
			details::borrowed_utf16_split_delimiter{ sv.base() });
	}
	...
	[[nodiscard]]
	constexpr auto split_terminator(utf16_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		using accessor_type = details::split_terminator_view_accessor<utf16_char>;
		return details::owning_string_view<Derived, accessor_type>{
			std::move(static_cast<Derived&>(*this)),
			accessor_type{ ch }
		};
	...
	{
		return utf16_rsplit_char_view<View, true>::from_delimiter_storage(
			code_unit_view(),
			details::owned_utf16_split_char_delimiter{ ch });
	}

	[[nodiscard]]
	constexpr auto rsplit_terminator(utf16_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		using accessor_type = details::rsplit_terminator_view_accessor<utf16_char>;
		return details::owning_string_view<Derived, accessor_type>{
			std::move(static_cast<Derived&>(*this)),
			accessor_type{ ch }
		};
	...
	{
		return utf16_rsplit_view<View, true>::from_delimiter_storage(
			code_unit_view(),
			details::borrowed_utf16_split_delimiter{ sv.base() });
	}
	...
	[[nodiscard]]
	constexpr auto splitn(size_type count, utf16_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		using accessor_type = details::splitn_view_accessor<utf16_char>;
		return details::owning_string_view<Derived, accessor_type>{
			std::move(static_cast<Derived&>(*this)),
	...
	[[nodiscard]]
	constexpr auto split_inclusive(utf16_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		using accessor_type = details::split_inclusive_view_accessor<utf16_char>;
		return details::owning_string_view<Derived, accessor_type>{
			std::move(static_cast<Derived&>(*this)),
			accessor_type{ ch }
		};
	...
	constexpr auto rsplitn(size_type count, utf16_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		using accessor_type = details::rsplitn_view_accessor<utf16_char>;
		return details::owning_string_view<Derived, accessor_type>{
			std::move(static_cast<Derived&>(*this)),
			accessor_type{ count, ch }
		};
	...
	[[nodiscard]]
	constexpr split_once_result<View> split_once(utf16_char ch) const& noexcept
	{
	...
	constexpr split_once_result<View> split_once(View sv) const& noexcept
	{
		...
		if (pos == npos)
		{
			return split_once_result<View>::failure(View::from_code_units_unchecked(code_units));
		}
	...
	[[nodiscard]]
	constexpr split_once_result<View> split_once(std::span<const utf16_char> chars) const& noexcept
	{
		if (chars.empty())
		{
			const auto code_units = code_unit_view();
	...
	[[nodiscard]]
	constexpr split_once_result<View> rsplit_once(utf16_char ch) const& noexcept
	{
	...
	[[nodiscard]]
	constexpr split_once_result<View> rsplit_once(std::span<const utf16_char> chars) const& noexcept
	{
		if (chars.empty())
		{
			const auto code_units = code_unit_view();
	...
	constexpr split_once_at_result<View> split_once_at(size_type delim) const& noexcept
	{
		...
		{
			return split_once_at_result<View>::failure(View::from_code_units_unchecked(code_unit_view()));
		}
	...
	[[nodiscard]]
	constexpr Derived trim_prefix(utf16_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		auto trimmed = trim_prefix_view(View::from_code_units_unchecked(details::utf16_char_view(ch)));
	...
	[[nodiscard]]
	constexpr auto trim_suffix(utf16_char ch) const& noexcept(std::same_as<Derived, View>)
	{
		return trim_suffix(View::from_code_units_unchecked(details::utf16_char_view(ch)));
	}

	[[nodiscard]]
	constexpr Derived trim_suffix(utf16_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		auto trimmed = trim_suffix_view(View::from_code_units_unchecked(details::utf16_char_view(ch)));
		return std::move(*this).move_owned_view(trimmed);
	}
	...
	[[nodiscard]]
	constexpr auto trim_start_matches(utf16_char ch) const& noexcept(std::same_as<Derived, View>)
	{
		return view_or_owned(trim_start_matches_view(ch));
	}

	[[nodiscard]]
	constexpr Derived trim_start_matches(utf16_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
	...
	[[nodiscard]]
	constexpr auto trim_start_matches(std::span<const utf16_char> chars) const& noexcept(std::same_as<Derived, View>)
	{
		return view_or_owned(trim_start_matches_view(chars));
	}

	[[nodiscard]]
	constexpr Derived trim_start_matches(std::span<const utf16_char> chars) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
	...
	[[nodiscard]]
	constexpr auto trim_end_matches(utf16_char ch) const& noexcept(std::same_as<Derived, View>)
	{
		return view_or_owned(trim_end_matches_view(ch));
	}

	[[nodiscard]]
	constexpr Derived trim_end_matches(utf16_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
	...
	[[nodiscard]]
	constexpr auto trim_end_matches(std::span<const utf16_char> chars) const& noexcept(std::same_as<Derived, View>)
	{
		return view_or_owned(trim_end_matches_view(chars));
	}

	[[nodiscard]]
	constexpr Derived trim_end_matches(std::span<const utf16_char> chars) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
	...
	[[nodiscard]]
	constexpr auto trim_matches(utf16_char ch) const& noexcept(std::same_as<Derived, View>)
	{
		return view_or_owned(trim_matches_view(ch));
	}

	[[nodiscard]]
	constexpr Derived trim_matches(utf16_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
	...
	[[nodiscard]]
	constexpr auto trim_matches(std::span<const utf16_char> chars) const& noexcept(std::same_as<Derived, View>)
	{
		return view_or_owned(trim_matches_view(chars));
	}

	[[nodiscard]]
	constexpr Derived trim_matches(std::span<const utf16_char> chars) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
...
protected:
		...
		requires (!std::same_as<Derived, View>)
	{
		...
		const auto pos = static_cast<size_type>(subrange.data() - code_units.data());
	...
	[[nodiscard]]
	constexpr View trim_start_matches_view(std::span<const utf16_char> chars) const noexcept
	{
		if (chars.empty())
		{
			return view_from_whole_string();
		...
		return trim_start_matches_view(details::utf16_char_span_matcher{ chars });
	}
	...
	[[nodiscard]]
	constexpr View trim_end_matches_view(utf16_char ch) const noexcept
	{
	...
	[[nodiscard]]
	constexpr View trim_end_matches_view(std::span<const utf16_char> chars) const noexcept
	{
		if (chars.empty())
		{
			return view_from_whole_string();
		...
		return trim_end_matches_view(details::utf16_char_span_matcher{ chars });
	}
	...
	[[nodiscard]]
	constexpr View trim_matches_view(utf16_char ch) const noexcept
	{
	...
	[[nodiscard]]
	constexpr View trim_matches_view(std::span<const utf16_char> chars) const noexcept
	{
...
namespace std::ranges
{
	...
	template <typename View, bool DropTrailingEmpty>
	inline constexpr bool enable_borrowed_range<
		unicode_ranges::details::basic_utf16_rsplit_view<
			View,
			DropTrailingEmpty,
			unicode_ranges::details::borrowed_utf16_split_delimiter>> = true;
