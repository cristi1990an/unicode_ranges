...
class basic_utf8_string : public details::utf8_string_crtp<basic_utf8_string<Allocator>, utf8_string_view>
{
 ...
 public:
	...
	static constexpr auto from_bytes(std::wstring_view bytes, const Allocator& alloc = Allocator())
		-> std::expected<basic_utf8_string, wide_string_error>
...
private:
	...
	{
		return utf16_inserted_utf8_size(code_units, base_.max_size() - base_.size());
	}

	[[nodiscard]]
	constexpr size_type utf16_inserted_utf8_size(std::u16string_view code_units, size_type max_output_size) const
	{
		size_type inserted_size = 0;
		if (!std::is_constant_evaluated())
		{
			if (auto output_size = details::simdutf_utf8_length_from_valid_utf16_if_available(code_units))
			{
		...
		while (read_index < code_units.size())
		{
			...
			add_output_size(inserted_size, utf8_code_unit_count(scalar), max_output_size);
			read_index += count;
		}
	...
	{
		return utf32_inserted_utf8_size(code_points, base_.max_size() - base_.size());
	}

	[[nodiscard]]
	constexpr size_type utf32_inserted_utf8_size(std::u32string_view code_points, size_type max_output_size) const
	{
		size_type inserted_size = 0;
		if (!std::is_constant_evaluated())
		{
		...
		for (char32_t code_point : code_points)
		{
			add_output_size(
				inserted_size,
				utf8_code_unit_count(static_cast<std::uint32_t>(code_point)),
				max_output_size);
		}
	...
	template <typename Writer>
	constexpr basic_utf8_string& replace_gap_and_write(size_type pos, size_type count, size_type replacement_size, Writer writer)
	{
		const auto old_size = base_.size();
		...
		if (new_size > old_size)
		{
			base_.resize_and_overwrite(new_size,
				[&](char8_t* buffer, std::size_t) noexcept
				{
					std::char_traits<char8_t>::move(buffer + pos + replacement_size, buffer + tail_pos, tail_size);
	...
	[[nodiscard]]
	constexpr size_type checked_replace_range_count(size_type pos, size_type count) const
	{
	...
	[[nodiscard]]
	constexpr size_type checked_replace_char_count(size_type pos) const
	{
		if (pos >= size()) [[unlikely]]
		{
			throw std::out_of_range("replace index out of range");
		...
		return this->char_at_unchecked(pos).code_unit_count();
	}

	template <typename Writer>
	[[nodiscard]]
	constexpr basic_utf8_string replace_copy_gap_and_write(
		size_type pos,
		size_type count,
		...
		Writer writer) const
	{
		...
		basic_utf8_string result{ base_.get_allocator() };
		result.base_.reserve(new_size);
		if (pos != 0)
		{
			result.base_.append(base_.data(), pos);
		}

		result.base_.resize(pos + replacement_size);

		[[maybe_unused]] const auto written = static_cast<size_type>(writer(result.base_.data() + pos));
		UTF8_RANGES_DEBUG_ASSERT(written == replacement_size);

		if (tail_size != 0)
		{
			result.base_.append(base_.data() + tail_pos, tail_size);
		}

		UTF8_RANGES_DEBUG_ASSERT(result.base_.size() == new_size);
		return result;
	}
	...
	static constexpr size_type write_utf16_as_utf8(std::u16string_view code_units, char8_t* out) noexcept
	{
		if (!std::is_constant_evaluated())
		{
			if (auto converted = details::simdutf_convert_valid_utf16_to_utf8_if_available(code_units, out))
			{
	...
	static constexpr size_type write_utf32_as_utf8(std::u32string_view code_points, char8_t* out) noexcept
	{
		if (!std::is_constant_evaluated())
		{
			if (auto converted = details::simdutf_convert_valid_utf32_to_utf8_if_available(code_points, out))
			{
				return static_cast<size_type>(*converted);
			}
		}
	...
	constexpr basic_utf8_string& insert_bytes(size_type index, equivalent_string_view bytes)
	{
		...
		return insert_gap_and_write(index, bytes.size(),
	...
	constexpr basic_utf8_string& replace_bytes(size_type pos, size_type count, equivalent_string_view bytes)
	{
		...
		return replace_gap_and_write(pos, count, bytes.size(),
			[&](char8_t* buffer) noexcept
			{
				std::char_traits<char8_t>::copy(buffer, bytes.data(), bytes.size());
				return bytes.size();
	...
	constexpr basic_utf8_string& append_owned_reversed_utf8_chars(views::owning_reversed_chars_view<basic_utf8_string>&& rg)
	{
		...
		return insert_gap_and_write(size(), inserted.size(),
			[&](char8_t* buffer) noexcept
			{
				std::char_traits<char8_t>::copy(buffer, inserted.data(), inserted.size());
	...
	constexpr basic_utf8_string& replace_owned_utf8_chars(size_type pos, size_type count, views::owning_chars_view<basic_utf8_string>&& rg)
	{
		auto owner = details::release_owned_string_view_owner(std::move(rg));
		const auto& source = owner.base();
		...
		return replace_bytes(pos, count, equivalent_string_view{ source });
	}

	constexpr basic_utf8_string& replace_owned_reversed_utf8_chars(
		size_type pos,
		size_type count,
		views::owning_reversed_chars_view<basic_utf8_string>&& rg)
	{
		...
		return replace_gap_and_write(pos, count, replacement.size(),
			[&](char8_t* buffer) noexcept
			{
				std::char_traits<char8_t>::copy(buffer, replacement.data(), replacement.size());
	...
	[[nodiscard]]
	constexpr base_type materialize_utf8_char_replacement(R&& rg) const
	{
			...
			base_.resize_and_overwrite(source_size,
				[&](char8_t* buffer, std::size_t) noexcept
				{
					...
					while (replacements_done != count)
					{
						...
						std::char_traits<char8_t>::move(buffer + write_index, buffer + cursor, prefix_size);
						write_index += prefix_size;
						std::char_traits<char8_t>::copy(buffer + write_index, replacement.data(), replacement.size());
						write_index += replacement.size();
						cursor = match + needle.size();
						++replacements_done;
					}

					std::char_traits<char8_t>::move(buffer + write_index, buffer + cursor, source_size - cursor);
		...
		if (output_size <= base_.capacity())
		{
			...
			base_.resize_and_overwrite(output_size,
				[&](char8_t* buffer, std::size_t) noexcept
				{
					...
					std::char_traits<char8_t>::move(buffer + write_end, buffer + selected_end, suffix_size);
					...
					while (replacements_remaining != 0)
					{
						...
						write_end -= tail_size;
						std::char_traits<char8_t>::move(buffer + write_end, buffer + tail_start, tail_size);
						write_end -= replacement.size();
						std::char_traits<char8_t>::copy(buffer + write_end, replacement.data(), replacement.size());
						read_end = match;
...
public:
	...
	constexpr basic_utf8_string& append_range(R&& rg)
	{
		...
		const auto appended = materialize_utf8_char_replacement(std::forward<R>(rg));
		return append_bytes(equivalent_string_view{ appended });
	}
	...
	constexpr basic_utf8_string& insert_range(size_type index, R&& rg)
	{
		...
		const auto inserted = materialize_utf8_char_replacement(std::forward<R>(rg));
		return insert_gap_and_write(index, inserted.size(),
			[&](char8_t* buffer) noexcept
			{
				std::char_traits<char8_t>::copy(buffer, inserted.data(), inserted.size());
	...
	{
		const auto bytes = std::u8string_view{ base_ };
		if (details::is_ascii_only(bytes)
			|| (form == normalization_form::nfc && details::nfc_quick_check_pass(bytes)))
		{
			return std::move(*this);
		}
	...
	template <details::utf8_char_predicate Pred>
	[[nodiscard]]
	constexpr basic_utf8_string replace_n(size_type count, Pred pred, utf8_char to) &&
	{
		return static_cast<const crtp&>(*this).template replace_n<std::remove_cvref_t<Pred>, Allocator>(
			count,
			std::move(pred),
	...
	template <details::utf8_char_predicate Pred>
	[[nodiscard]]
	constexpr basic_utf8_string replace_n(size_type count, Pred pred, utf8_string_view to) &&
	{
		return static_cast<const crtp&>(*this).template replace_n<std::remove_cvref_t<Pred>, Allocator>(
			count,
			std::move(pred),
	...
	[[nodiscard]]
	constexpr basic_utf8_string replace_at(
		size_type pos,
		size_type count,
		views::owning_reversed_chars_view<basic_utf8_string>&& rg) const&
	{
		auto result = *this;
		result.replace_at_mutating(pos, count, std::move(rg));
		return result;
	...
	{
		const auto replace_count = checked_replace_range_count(pos, count);
		const auto code_units = rg.base();
		const auto retained_size = size() - replace_count;
		const auto replacement_size = utf16_inserted_utf8_size(code_units, base_.max_size() - retained_size);
		return replace_copy_gap_and_write(
			pos,
			replace_count,
			replacement_size,
			[&](char8_t* buffer) noexcept
	...
	constexpr basic_utf8_string replace_at(size_type pos, size_type count, views::utf32_view rg) const&
	{
		...
		const auto code_points = rg.base();
		const auto retained_size = size() - replace_count;
		const auto replacement_size = utf32_inserted_utf8_size(code_points, base_.max_size() - retained_size);
		return replace_copy_gap_and_write(
			pos,
			replace_count,
			replacement_size,
			[&](char8_t* buffer) noexcept
			{
				return write_utf32_as_utf8(code_points, buffer);
			});
	}
	...
	{
		const auto replace_count = checked_replace_char_count(pos);
		const auto code_units = rg.base();
		const auto retained_size = size() - replace_count;
		const auto replacement_size = utf16_inserted_utf8_size(code_units, base_.max_size() - retained_size);
		return replace_copy_gap_and_write(
...
private:
	...
	constexpr basic_utf8_string& replace_at_mutating(size_type pos, size_type count, utf8_string_view other)
	{
		if (pos > size()) [[unlikely]]
		{
			throw std::out_of_range("replace index out of range");
	...
	constexpr basic_utf8_string& replace_at_mutating(size_type pos, size_type count, utf8_char other)
	{
		return replace_at_mutating(pos, count, utf8_string_view::from_bytes_unchecked(details::utf8_char_view(other)));
	}

	constexpr basic_utf8_string& replace_at_mutating(size_type pos, utf8_string_view other)
	{
		if (pos >= size()) [[unlikely]]
		{
			throw std::out_of_range("replace index out of range");
	...
	constexpr basic_utf8_string& replace_at_mutating(size_type pos, utf8_char other)
	{
		if (pos >= size()) [[unlikely]]
		{
			throw std::out_of_range("replace index out of range");
	...
	constexpr basic_utf8_string& replace_at_mutating(size_type pos, size_type count, views::utf8_view rg)
	{
		if (pos > size()) [[unlikely]]
		{
			throw std::out_of_range("replace index out of range");
	...
	constexpr basic_utf8_string& replace_at_mutating(size_type pos, size_type count, views::owning_chars_view<basic_utf8_string>&& rg)
	{
		if (pos > size()) [[unlikely]]
		{
			throw std::out_of_range("replace index out of range");
		...
		return replace_owned_utf8_chars(pos, replace_count, std::move(rg));
	}

	constexpr basic_utf8_string& replace_at_mutating(
		size_type pos,
		size_type count,
		views::owning_reversed_chars_view<basic_utf8_string>&& rg)
	{
		...
		return replace_owned_reversed_utf8_chars(pos, replace_count, std::move(rg));
	}

	constexpr basic_utf8_string& replace_at_mutating(size_type pos, size_type count, views::utf16_view rg)
	{
		if (pos > size()) [[unlikely]]
		{
			throw std::out_of_range("replace index out of range");
	...
	constexpr basic_utf8_string& replace_at_mutating(size_type pos, size_type count, views::utf32_view rg)
	{
		if (pos > size()) [[unlikely]]
		{
			throw std::out_of_range("replace index out of range");
	...
	template <details::container_compatible_range<utf8_char> R>
		requires (!optimized_utf8_chars_range<R>)
	constexpr basic_utf8_string& replace_at_mutating(size_type pos, size_type count, R&& rg)
	{
		if (pos > size()) [[unlikely]]
		{
			throw std::out_of_range("replace index out of range");
		...
		if constexpr (details::contiguous_sized_range_of<R, utf8_char>)
		{
			...
			const auto replacement_size = static_cast<size_type>(
				details::utf8_char_sequence_code_unit_count(chars, char_count));
			return replace_gap_and_write(pos, replace_count, replacement_size,
				[&](char8_t* buffer) noexcept
				{
					details::copy_utf8_char_sequence(chars, char_count, buffer);
					return replacement_size;
				});
		}

		const auto replacement = materialize_utf8_char_replacement(std::forward<R>(rg));
	...
	constexpr basic_utf8_string& replace_at_mutating(size_type pos, views::utf8_view rg)
	{
		if (pos >= size()) [[unlikely]]
		{
			throw std::out_of_range("replace index out of range");
	...
	constexpr basic_utf8_string& replace_at_mutating(size_type pos, views::owning_chars_view<basic_utf8_string>&& rg)
	{
		if (pos >= size()) [[unlikely]]
		{
			throw std::out_of_range("replace index out of range");
	...
	constexpr basic_utf8_string& replace_at_mutating(size_type pos, views::owning_reversed_chars_view<basic_utf8_string>&& rg)
	{
		if (pos >= size()) [[unlikely]]
		{
			throw std::out_of_range("replace index out of range");
	...
	constexpr basic_utf8_string& replace_at_mutating(size_type pos, views::utf16_view rg)
	{
		if (pos >= size()) [[unlikely]]
		{
			throw std::out_of_range("replace index out of range");
	...
	constexpr basic_utf8_string& replace_at_mutating(size_type pos, views::utf32_view rg)
	{
		if (pos >= size()) [[unlikely]]
		{
			throw std::out_of_range("replace index out of range");
	...
	template <details::container_compatible_range<utf8_char> R>
		requires (!optimized_utf8_chars_range<R>)
	constexpr basic_utf8_string& replace_at_mutating(size_type pos, R&& rg)
	{
		if (pos >= size()) [[unlikely]]
		{
			throw std::out_of_range("replace index out of range");
...
public:
	...
	constexpr size_type copy(char8_t* dest, size_type count, size_type pos = 0) const
	{
		...
		if (copied != 0)
		{
			std::char_traits<char8_t>::copy(dest, base_.data() + pos, copied);
		}
		return copied;
	}
	...
	friend constexpr void swap(basic_utf8_string& lhs, basic_utf8_string& rhs)
	...
	{
		lhs.append_bytes(equivalent_string_view{ rhs.base_ });
		return std::move(lhs);
	}

	friend constexpr basic_utf8_string operator+(const basic_utf8_string& lhs, basic_utf8_string&& rhs)
	{
		rhs.insert_bytes(0, equivalent_string_view{ lhs.base_ });
		return std::move(rhs);
	}
	...
	{
		lhs.append_bytes(details::utf8_char_view(rhs));
		return std::move(lhs);
	}
	...
	{
		rhs.insert_bytes(0, details::utf8_char_view(lhs));
		return std::move(rhs);
	}
