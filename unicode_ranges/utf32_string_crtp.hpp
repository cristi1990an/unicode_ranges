...
inline constexpr bool utf32_split_delimiter_has_self_overlap(std::u32string_view delimiter) noexcept
{
...
inline constexpr bool utf32_split_delimiter_has_overlapping_predecessor(
	std::u32string_view base,
	std::u32string_view delimiter,
	...
	: public std::ranges::view_interface<basic_utf32_split_view<View, DropTrailingEmpty, DelimiterStorage>>
{
	...
	friend class basic_utf32_rsplit_view;
...
private:
		...
		std::size_t current) noexcept
	{
		const auto delimiter_pos = [&]() constexpr noexcept {
			...
			{
				return details::rfind_utf32_split_delimiter(base, delimiter, base.size());
			}
			...
			return details::rfind_utf32_split_delimiter(base, delimiter, current - 1);
		}();
		if (delimiter_pos == std::u32string_view::npos)
		{
			return 0;
		}

		if (!details::utf32_split_delimiter_has_self_overlap(delimiter)
			|| !details::utf32_split_delimiter_has_overlapping_predecessor(base, delimiter, delimiter_pos))
		{
...
template <typename View, bool DropTrailingEmpty, typename DelimiterStorage>
class basic_utf32_rsplit_view
	: public std::ranges::view_interface<basic_utf32_rsplit_view<View, DropTrailingEmpty, DelimiterStorage>>
{
public:
	static constexpr basic_utf32_rsplit_view from_delimiter_storage(
		std::u32string_view base,
		DelimiterStorage delimiter_storage) noexcept
	{
		return basic_utf32_rsplit_view{ base, delimiter_storage };
	}
	...
	public:
		...
		constexpr reference operator*() const noexcept
		{
			return View::from_code_points_unchecked(base_.substr(current_, segment_end_ - current_));
		}

		constexpr iterator& operator++() noexcept
		{
			if (current_ == 0 || current_ == std::u32string_view::npos)
			{
				current_ = std::u32string_view::npos;
				segment_end_ = std::u32string_view::npos;
				return *this;
			}

			segment_end_ = current_ - delimiter_.size();
			current_ = basic_utf32_split_view<View, DropTrailingEmpty, DelimiterStorage>::find_previous_segment_start(
				base_,
				delimiter_,
				segment_end_);
			return *this;
		...
		constexpr iterator& operator--() noexcept
		{
			...
			{
				current_ = 0;
				const auto next = details::find_utf32_split_delimiter(base_, delimiter_, 0);
				segment_end_ = next == std::u32string_view::npos ? base_.size() : next;
				return *this;
			}
			...
			const auto next = details::find_utf32_split_delimiter(base_, delimiter_, current_);
			segment_end_ = next == std::u32string_view::npos ? base_.size() : next;
			return *this;
		}
	...
	private:
		...
		std::size_t segment_end_ = std::u32string_view::npos;
	};
	...
	constexpr iterator begin() const noexcept
	{
		...
		const auto segment_end = [&]() constexpr noexcept {
			if constexpr (DropTrailingEmpty)
			{
				if (details::utf32_split_input_ends_with_delimiter(base_, delimiter))
				{
		...
		const auto current = segment_end == base_.size()
			? basic_utf32_split_view<View, DropTrailingEmpty, DelimiterStorage>::find_previous_segment_start(
				base_,
				delimiter,
				std::u32string_view::npos)
			: basic_utf32_split_view<View, DropTrailingEmpty, DelimiterStorage>::find_previous_segment_start(
...
private:
	constexpr explicit basic_utf32_rsplit_view(
		std::u32string_view base,
		DelimiterStorage delimiter_storage) noexcept
		: base_(base), delimiter_storage_(delimiter_storage)
	{}
...
template <typename View, bool DropTrailingEmpty>
using utf32_rsplit_view = basic_utf32_rsplit_view<View, DropTrailingEmpty, borrowed_utf32_split_delimiter>;

template <typename View, bool DropTrailingEmpty>
using utf32_rsplit_char_view = basic_utf32_rsplit_view<View, DropTrailingEmpty, owned_utf32_split_char_delimiter>;
...
private:
		...
		std::size_t current) noexcept
	{
		auto previous = [&]() constexpr noexcept {
			if (current == std::u32string_view::npos)
			{
				return details::rfind_utf32_predicate_match(base, base.size(), pred);
			}

			if (current == 0)
			{
				return utf32_predicate_match{};
			}

			auto match = details::rfind_utf32_predicate_match(base, current, pred);
			while (match.pos != std::u32string_view::npos && match.pos + match.size >= current)
			{
				match = details::rfind_utf32_predicate_match(base, match.pos, pred);
			}
			return match;
		}();
		return previous.pos == std::u32string_view::npos ? 0 : previous.pos + previous.size;
	}
...
private:
	...
	static constexpr bool predicate_noexcept =
		noexcept(std::invoke(std::declval<Pred&>(), std::declval<utf32_char>()));
...
public:
	...
	[[nodiscard]]
	constexpr Derived to_utf32() && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>);
	template <typename Allocator>
	[[nodiscard]]
	constexpr basic_utf32_string<Allocator> to_utf32(const Allocator& alloc) &&
		requires (!std::same_as<Derived, View>);
	template <typename Allocator = std::allocator<char32_t>>
	[[nodiscard]]
	...
	{
		return utf32_rsplit_char_view<View, false>::from_delimiter_storage(
			code_unit_view(),
			details::owned_utf32_split_char_delimiter{ ch });
	}
	...
	{
		return utf32_rsplit_view<View, false>::from_delimiter_storage(
			code_unit_view(),
			details::borrowed_utf32_split_delimiter{ sv.base() });
	}
	...
	{
		return utf32_rsplit_char_view<View, true>::from_delimiter_storage(
			code_unit_view(),
			details::owned_utf32_split_char_delimiter{ ch });
	}
	...
	{
		return utf32_rsplit_view<View, true>::from_delimiter_storage(
			code_unit_view(),
			details::borrowed_utf32_split_delimiter{ sv.base() });
	}
	...
	constexpr auto matches(utf32_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		using accessor_type = details::matches_view_accessor<utf32_char>;
		return details::owning_string_view<Derived, accessor_type>{
			std::move(static_cast<Derived&>(*this)),
			accessor_type{ ch }
		};
	...
	template <std::ranges::viewable_range R>
		requires details::char_set_viewable_range<R, utf32_char>
	constexpr auto matches(R&& chars) const& noexcept(noexcept(details::make_char_set_matcher<utf32_char>(std::forward<R>(chars))))
	{
		return matches(details::make_char_set_matcher<utf32_char>(std::forward<R>(chars)));
	}

	template <std::ranges::viewable_range R>
		requires details::char_set_viewable_range<R, utf32_char>
	constexpr auto matches(R&& chars) && noexcept(std::is_nothrow_move_constructible_v<Derived>
		&& noexcept(details::make_char_set_matcher<utf32_char>(std::forward<R>(chars))))
		requires (!std::same_as<Derived, View>)
	{
		auto matcher = details::make_char_set_matcher<utf32_char>(std::forward<R>(chars));
	...
	constexpr auto match_indices(utf32_char ch) const& noexcept
	{
		return utf32_match_indices_char_view<View, false>::from_delimiter_storage(
			code_unit_view(),
			details::owned_utf32_split_char_delimiter{ ch });
	...
	constexpr auto match_indices(utf32_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		using accessor_type = details::match_indices_view_accessor<utf32_char>;
		return details::owning_string_view<Derived, accessor_type>{
			std::move(static_cast<Derived&>(*this)),
			accessor_type{ ch }
		};
	...
	template <std::ranges::viewable_range R>
		requires details::char_set_viewable_range<R, utf32_char>
	constexpr auto match_indices(R&& chars) const& noexcept(noexcept(details::make_char_set_matcher<utf32_char>(std::forward<R>(chars))))
	{
		return match_indices(details::make_char_set_matcher<utf32_char>(std::forward<R>(chars)));
	}
	...
	constexpr auto rmatches(utf32_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		using accessor_type = details::rmatches_view_accessor<utf32_char>;
		return details::owning_string_view<Derived, accessor_type>{
			std::move(static_cast<Derived&>(*this)),
			accessor_type{ ch }
		};
	...
	template <std::ranges::viewable_range R>
		requires details::char_set_viewable_range<R, utf32_char>
	constexpr auto rmatches(R&& chars) const& noexcept(noexcept(details::make_char_set_matcher<utf32_char>(std::forward<R>(chars))))
	{
		return rmatches(details::make_char_set_matcher<utf32_char>(std::forward<R>(chars)));
	}
	...
	constexpr auto rmatch_indices(utf32_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		using accessor_type = details::rmatch_indices_view_accessor<utf32_char>;
		return details::owning_string_view<Derived, accessor_type>{
			std::move(static_cast<Derived&>(*this)),
			accessor_type{ ch }
		};
	...
	template <std::ranges::viewable_range R>
		requires details::char_set_viewable_range<R, utf32_char>
	constexpr auto rmatch_indices(R&& chars) const& noexcept(noexcept(details::make_char_set_matcher<utf32_char>(std::forward<R>(chars))))
	{
		return rmatch_indices(details::make_char_set_matcher<utf32_char>(std::forward<R>(chars)));
	}
	...
	[[nodiscard]] constexpr basic_utf32_string<> replace_at(size_type pos, size_type count, utf32_char other) const;
	[[nodiscard]] constexpr basic_utf32_string<> replace_at(size_type pos, size_type count, View other) const;
	[[nodiscard]] constexpr basic_utf32_string<> replace_at(size_type pos, utf32_char other) const;
	[[nodiscard]] constexpr basic_utf32_string<> replace_at(size_type pos, View other) const;

	template <typename Allocator>
	[[nodiscard]]
	constexpr basic_utf32_string<Allocator> replace_at(size_type pos, size_type count, utf32_char other, const Allocator& alloc) const;

	template <typename Allocator>
	[[nodiscard]]
	constexpr basic_utf32_string<Allocator> replace_at(size_type pos, size_type count, View other, const Allocator& alloc) const;

	template <typename Allocator>
	[[nodiscard]]
	constexpr basic_utf32_string<Allocator> replace_at(size_type pos, utf32_char other, const Allocator& alloc) const;

	template <typename Allocator>
	[[nodiscard]]
	constexpr basic_utf32_string<Allocator> replace_at(size_type pos, View other, const Allocator& alloc) const;
	...
	[[nodiscard]]
	constexpr auto strip_prefix(utf32_char ch) const& noexcept(std::same_as<Derived, View>)
	{
		return strip_prefix(View::from_code_points_unchecked(details::utf32_char_view(ch)));
	}

	[[nodiscard]]
	constexpr std::optional<Derived> strip_prefix(utf32_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		auto stripped = strip_prefix_view(View::from_code_points_unchecked(details::utf32_char_view(ch)));
	...
	[[nodiscard]]
	constexpr auto strip_suffix(utf32_char ch) const& noexcept(std::same_as<Derived, View>)
	{
		return strip_suffix(View::from_code_points_unchecked(details::utf32_char_view(ch)));
	}

	[[nodiscard]]
	constexpr std::optional<Derived> strip_suffix(utf32_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		auto stripped = strip_suffix_view(View::from_code_points_unchecked(details::utf32_char_view(ch)));
		return std::move(*this).move_optional_owned_view(stripped);
	}
	...
	[[nodiscard]]
	constexpr auto strip_circumfix(utf32_char prefix, utf32_char suffix) const& noexcept(std::same_as<Derived, View>)
	{
	...
	[[nodiscard]]
	constexpr std::optional<Derived> strip_circumfix(utf32_char prefix, utf32_char suffix) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
	...
	[[nodiscard]]
	constexpr auto trim_prefix(utf32_char ch) const& noexcept(std::same_as<Derived, View>)
	{
		return trim_prefix(View::from_code_points_unchecked(details::utf32_char_view(ch)));
	}

	[[nodiscard]]
	constexpr Derived trim_prefix(utf32_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		auto trimmed = trim_prefix_view(View::from_code_points_unchecked(details::utf32_char_view(ch)));
	...
	[[nodiscard]]
	constexpr auto trim_suffix(utf32_char ch) const& noexcept(std::same_as<Derived, View>)
	{
		return trim_suffix(View::from_code_points_unchecked(details::utf32_char_view(ch)));
	}

	[[nodiscard]]
	constexpr Derived trim_suffix(utf32_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
		auto trimmed = trim_suffix_view(View::from_code_points_unchecked(details::utf32_char_view(ch)));
		return std::move(*this).move_owned_view(trimmed);
	}
	...
	[[nodiscard]]
	constexpr auto trim_start_matches(utf32_char ch) const& noexcept(std::same_as<Derived, View>)
	{
		return view_or_owned(trim_start_matches_view(ch));
	}

	[[nodiscard]]
	constexpr Derived trim_start_matches(utf32_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
	...
	[[nodiscard]]
	constexpr auto trim_start_matches(std::span<const utf32_char> chars) const& noexcept(std::same_as<Derived, View>)
	{
		return view_or_owned(trim_start_matches_view(chars));
	}

	[[nodiscard]]
	constexpr Derived trim_start_matches(std::span<const utf32_char> chars) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
	...
	[[nodiscard]]
	constexpr auto trim_end_matches(utf32_char ch) const& noexcept(std::same_as<Derived, View>)
	{
		return view_or_owned(trim_end_matches_view(ch));
	}

	[[nodiscard]]
	constexpr Derived trim_end_matches(utf32_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
	...
	[[nodiscard]]
	constexpr auto trim_end_matches(std::span<const utf32_char> chars) const& noexcept(std::same_as<Derived, View>)
	{
		return view_or_owned(trim_end_matches_view(chars));
	}

	[[nodiscard]]
	constexpr Derived trim_end_matches(std::span<const utf32_char> chars) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
	...
	[[nodiscard]]
	constexpr auto trim_matches(utf32_char ch) const& noexcept(std::same_as<Derived, View>)
	{
		return view_or_owned(trim_matches_view(ch));
	}

	[[nodiscard]]
	constexpr Derived trim_matches(utf32_char ch) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
	...
	[[nodiscard]]
	constexpr auto trim_matches(std::span<const utf32_char> chars) const& noexcept(std::same_as<Derived, View>)
	{
		return view_or_owned(trim_matches_view(chars));
	}

	[[nodiscard]]
	constexpr Derived trim_matches(std::span<const utf32_char> chars) && noexcept(std::is_nothrow_move_constructible_v<Derived>)
		requires (!std::same_as<Derived, View>)
	{
...
protected:
		...
		requires (!std::same_as<Derived, View>)
	{
		...
		const auto pos = static_cast<size_type>(subrange.data() - code_points.data());
		...
		requires (!std::same_as<Derived, View>)
	{
		...
		if (first != 0)
		{
			...
			if (retained_size != 0)
			{
				std::char_traits<char32_t>::move(text.base_.data(), text.base_.data() + first, retained_size);
			}
	...
	[[nodiscard]]
	constexpr View trim_start_matches_view(utf32_char ch) const noexcept
	{
	...
	[[nodiscard]]
	constexpr View trim_start_matches_view(std::span<const utf32_char> chars) const noexcept
	{
		if (chars.empty())
		{
			return view_from_whole_string();
		...
		return trim_start_matches_view(details::utf32_char_span_matcher{ chars });
	}
	...
	[[nodiscard]]
	constexpr View trim_end_matches_view(utf32_char ch) const noexcept
	{
	...
	[[nodiscard]]
	constexpr View trim_end_matches_view(std::span<const utf32_char> chars) const noexcept
	{
		if (chars.empty())
		{
			return view_from_whole_string();
		...
		return trim_end_matches_view(details::utf32_char_span_matcher{ chars });
	}
	...
	[[nodiscard]]
	constexpr View trim_matches_view(utf32_char ch) const noexcept
	{
	...
	[[nodiscard]]
	constexpr View trim_matches_view(std::span<const utf32_char> chars) const noexcept
	{
...
namespace std::ranges
{
	...
	template <typename View, bool DropTrailingEmpty>
	inline constexpr bool enable_borrowed_range<
		unicode_ranges::details::basic_utf32_rsplit_view<
			View,
			DropTrailingEmpty,
			unicode_ranges::details::borrowed_utf32_split_delimiter>> = true;
