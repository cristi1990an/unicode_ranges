...
private:
	template <typename R>
	static constexpr bool direct_utf32_chars_range =
		std::same_as<std::remove_cvref_t<R>, views::utf32_view>;

	template <typename R>
	static constexpr bool rvalue_owned_utf32_chars_range =
		std::same_as<std::remove_cvref_t<R>, views::owning_chars_view<basic_utf32_string>>
		&& !std::is_lvalue_reference_v<R>;

	template <typename R>
	static constexpr bool rvalue_owned_reversed_utf32_chars_range =
		std::same_as<std::remove_cvref_t<R>, views::owning_reversed_chars_view<basic_utf32_string>>
		&& !std::is_lvalue_reference_v<R>;

	template <typename R>
	static constexpr bool optimized_utf32_chars_range =
		direct_utf32_chars_range<R> || rvalue_owned_utf32_chars_range<R> || rvalue_owned_reversed_utf32_chars_range<R>;
	...
	{
		return utf8_inserted_utf32_size(bytes, base_.max_size() - base_.size());
	}

	[[nodiscard]]
	constexpr size_type utf8_inserted_utf32_size(std::u8string_view bytes, size_type max_output_size) const
	{
	...
	{
		return utf16_inserted_utf32_size(code_units, base_.max_size() - base_.size());
	}

	[[nodiscard]]
	constexpr size_type utf16_inserted_utf32_size(std::u16string_view code_units, size_type max_output_size) const
	{
		if (!std::is_constant_evaluated())
		{
			if (auto output_size = details::simdutf_utf32_length_from_valid_utf16_if_available(code_units))
			{
				if (*output_size > max_output_size) [[unlikely]]
				{
					throw std::length_error("insert size exceeds max_size");
	...
	static constexpr size_type write_utf16_as_utf32(std::u16string_view code_units, char32_t* out) noexcept
	{
		if (!std::is_constant_evaluated())
		{
			if (auto converted = details::simdutf_convert_valid_utf16_to_utf32_if_available(code_units, out))
			{
				return static_cast<size_type>(*converted);
			}
		}
	...
	constexpr basic_utf32_string& assign_owned_utf32_chars(views::owning_chars_view<basic_utf32_string>&& rg)
	{
	...
	constexpr basic_utf32_string& append_owned_utf32_chars(views::owning_chars_view<basic_utf32_string>&& rg)
	{
		auto owner = details::release_owned_string_view_owner(std::move(rg));
		const auto& source = owner.base();
		...
		return append_code_points(equivalent_string_view{ source });
	}

	constexpr basic_utf32_string& insert_owned_utf32_chars(size_type index, views::owning_chars_view<basic_utf32_string>&& rg)
	{
		auto owner = details::release_owned_string_view_owner(std::move(rg));
		const auto& source = owner.base();
		if (base_.empty() && can_steal_storage_from(source))
		...
		return insert_code_points(index, equivalent_string_view{ source });
	}

	constexpr basic_utf32_string& assign_owned_reversed_utf32_chars(views::owning_reversed_chars_view<basic_utf32_string>&& rg)
	{
		auto owner = details::release_owned_string_view_owner(std::move(rg));
		const auto& source = owner.base();
		if (can_steal_storage_from(source))
	...
	constexpr basic_utf32_string& append_owned_reversed_utf32_chars(views::owning_reversed_chars_view<basic_utf32_string>&& rg)
	{
		if (base_.empty())
		{
	...
	constexpr basic_utf32_string& replace_owned_utf32_chars(size_type pos, size_type count, views::owning_chars_view<basic_utf32_string>&& rg)
	{
		...
		return replace_code_points(pos, count, equivalent_string_view{ source });
	}

	constexpr basic_utf32_string& replace_owned_reversed_utf32_chars(
		size_type pos,
		size_type count,
			...
			base_.resize_and_overwrite(source_size,
				[&](char32_t* buffer, std::size_t) noexcept
				{
					...
					while (replacements_done != count)
					{
						...
						std::char_traits<char32_t>::move(buffer + write_index, buffer + cursor, prefix_size);
						write_index += prefix_size;
						std::char_traits<char32_t>::copy(buffer + write_index, replacement.data(), replacement.size());
						write_index += replacement.size();
						cursor = match + needle.size();
						++replacements_done;
					}

					std::char_traits<char32_t>::move(buffer + write_index, buffer + cursor, source_size - cursor);
		...
		if (output_size <= base_.capacity())
		{
			...
			base_.resize_and_overwrite(output_size,
				[&](char32_t* buffer, std::size_t) noexcept
				{
					...
					std::char_traits<char32_t>::move(buffer + write_end, buffer + selected_end, suffix_size);
					...
					while (replacements_remaining != 0)
					{
						...
						write_end -= tail_size;
						std::char_traits<char32_t>::move(buffer + write_end, buffer + tail_start, tail_size);
						write_end -= replacement.size();
						std::char_traits<char32_t>::copy(buffer + write_end, replacement.data(), replacement.size());
						read_end = match;
...
public:
	...
	constexpr basic_utf32_string(std::from_range_t, views::utf32_view rg, const Allocator& alloc = Allocator())
		: base_(alloc)
	{
		append_range(rg);
	}

	constexpr basic_utf32_string(
		std::from_range_t,
		views::owning_chars_view<basic_utf32_string>&& rg,
		const Allocator& alloc = Allocator())
		: base_(alloc)
	{
		assign_owned_utf32_chars(std::move(rg));
	}
	...
	constexpr basic_utf32_string(
		...
		: base_(alloc)
	{
		assign_owned_reversed_utf32_chars(std::move(rg));
	}
	...
	constexpr basic_utf32_string& append_range(views::owning_chars_view<basic_utf32_string>&& rg)
	{
		return append_owned_utf32_chars(std::move(rg));
	}

	constexpr basic_utf32_string& append_range(views::owning_reversed_chars_view<basic_utf32_string>&& rg)
	{
		return append_owned_reversed_utf32_chars(std::move(rg));
	}
	...
	constexpr basic_utf32_string& assign_range(views::owning_chars_view<basic_utf32_string>&& rg)
	{
		return assign_owned_utf32_chars(std::move(rg));
	}

	constexpr basic_utf32_string& assign_range(views::owning_reversed_chars_view<basic_utf32_string>&& rg)
	{
		return assign_owned_reversed_utf32_chars(std::move(rg));
	}
	...
	constexpr basic_utf32_string& insert_range(size_type index, views::owning_chars_view<basic_utf32_string>&& rg)
	{
		if (index > size()) [[unlikely]]
		{
			throw std::out_of_range("insert index out of range");
		...
		return insert_owned_utf32_chars(index, std::move(rg));
	}

	constexpr basic_utf32_string& insert_range(size_type index, views::owning_reversed_chars_view<basic_utf32_string>&& rg)
	{
		if (index > size()) [[unlikely]]
		{
			throw std::out_of_range("insert index out of range");
		...
		return insert_owned_reversed_utf32_chars(index, std::move(rg));
	}
	...
	{
		auto code_points = std::u32string_view{ base_ };
		if (details::is_ascii_only(code_points))
		{
			details::ascii_lowercase_inplace(base_.data(), base_.size());
			return std::move(*this);
		}

		if (details::case_map_utf32_inplace_if_same_size<true>(code_points, base_.data()))
		{
			return std::move(*this);
		}
	...
	constexpr basic_utf32_string to_uppercase() &&
	{
		...
		if (details::case_map_utf32_inplace_if_same_size<false>(code_points, base_.data()))
		{
			return std::move(*this);
		}
	...
	{
		const auto code_points = std::u32string_view{ base_ };
		if (details::is_ascii_only(code_points)
			|| (form == normalization_form::nfc && details::nfc_quick_check_pass(code_points)))
		{
			return std::move(*this);
		}
	...
	constexpr basic_utf32_string case_fold() &&
	{
		...
		if (details::case_fold_utf32_inplace_if_same_size(code_points, base_.data()))
		{
			return std::move(*this);
		}
...
public:
	...
	{
		lhs.append_code_points(equivalent_string_view{ rhs.base_ });
		return std::move(lhs);
	}

	friend constexpr basic_utf32_string operator+(const basic_utf32_string& lhs, basic_utf32_string&& rhs)
	{
		rhs.insert_code_points(0, equivalent_string_view{ lhs.base_ });
		return std::move(rhs);
	}
	...
	{
		lhs.append_code_points(rhs.base());
		return std::move(lhs);
	}
	...
	{
		rhs.insert_code_points(0, lhs.base());
		return std::move(rhs);
	}
	...
	{
		lhs.append_code_points(details::utf32_char_view(rhs));
		return std::move(lhs);
	}
	...
	{
		rhs.insert_code_points(0, details::utf32_char_view(lhs));
		return std::move(rhs);
	}
...
extern template class basic_utf32_string<std::allocator<char32_t>>;
extern template class basic_utf32_string<std::pmr::polymorphic_allocator<char32_t>>;
