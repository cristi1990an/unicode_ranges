...
The public error payloads share one shape. `utf8_error`, `utf16_error`, `utf32_error`, `unicode_scalar_error`, and `wide_string_error` are aliases of `unicode_error`; their `code` aliases share `unicode_error_code`. Use `first_invalid_element_index` to read the failing input element index.
...
Returns [`std::unexpected(...)`](https://en.cppreference.com/w/cpp/utility/expected/unexpected) with the operation-specific error alias when validation or transcoding fails. `std::wstring_view` overloads consistently use `wide_string_error` across platforms.
